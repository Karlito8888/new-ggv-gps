...
- Do NOT remove `ne2_shaded` raster source from style.json (natural earth background, harmless, works at low zoom for context)
- Do NOT change the tile source to Protomaps schema without also updating all style layers (80+ layers would need rewriting)
...
- Protomaps sprite URL (CDN) used for now; Story 1.3 (Workbox SW) will cache it for offline.
...
- `src/hooks/useMapSetup.js` — MODIFIED: PMTiles protocol registration, inline Protomaps style, lazy-load protomaps-themes-base
- `public/style/style.json` — DELETED: replaced by inline style generation
- `public/sw.js` — MODIFIED: removed stale `tiles.openfreemap.org` from TILE_HOSTS (tiles now served locally)
- `vite.config.js` — MODIFIED: pmtiles + protomaps-themes-base added to maps chunk
- `index.html` — MODIFIED: CSP updated (tiles.openfreemap.org removed, protomaps.github.io added), stale preconnect/preload removed
- `netlify.toml` — MODIFIED: added /tiles/* cache headers
