...
export function projectPointOnLine(pointLng, pointLat, lineCoordinates) {
  if (!lineCoordinates || lineCoordinates.length < 2) {
    return { projectedPoint: [pointLng, pointLat], segmentIndex: 0, progressOnSegment: 0, deviationDistance: 0 };
  }
...
export function getDistanceAlongRoute(userLng, userLat, targetLng, targetLat, routeCoordinates) {
  ...
  for (let i = 0; i < routeCoordinates.length - 1; i++) {
    ...
    let t = 0;
    if (segmentLengthSq > 0) {
      t = Math.max(0, Math.min(1, ((targetLng - x1) * dx + (targetLat - y1) * dy) / segmentLengthSq));
    }
  ...
  if (targetSegmentIndex === userProjection.segmentIndex && targetProgress < userProjection.progressOnSegment) {
