...
export function useRouting(map, origin, destination) {
  ...
  useEffect(() => {
    ...
    const calculateRoute = async () => {
      ...
      try {
        ...
        const osrmResult = await fetchOSRMRoute(origin, destination, abortControllerRef.current.signal);
  ...
  }, [map, origin?.latitude, origin?.longitude, destination?.coordinates?.[0], destination?.coordinates?.[1]]);
